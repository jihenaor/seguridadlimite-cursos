...
class scrollSpy extends Component {
        ...
        document.querySelector("div[data-nav='list']").querySelectorAll("a").forEach((navLink) => {
            navLink.addEventListener("click", (event) => {
              ...
              if (sectionID) {
                ...
                if (!targetElement) return; // <-- Previene el error si el elemento no existe
                ...
                const targetMarginTop = parseFloat(targetStyles.marginTop) || 0;
                const targetPaddingTop = parseFloat(targetStyles.paddingTop) || 0;
                let scrollTargetPosition = targetElement.offsetTop - navbarHeight - targetMarginTop - targetPaddingTop;
    ...
    scrollSection = () => {
        ...
        this.scrollTargetIds.forEach((sectionID, index) => {
            ...
            if (!sectionElement) return; // Evita el error si el elemento no existe

            scrollSectionOffsetTop = sectionElement.offsetTop - (this.headerBackground ? document.querySelector("div[data-nav='list']").scrollHeight : 0);

            if (
                window.pageYOffset >= scrollSectionOffsetTop &&
                window.pageYOffset < scrollSectionOffsetTop + sectionElement.scrollHeight
            ) {
